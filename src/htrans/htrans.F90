...
!!$ EMTO vars
   ...
   Integer :: i, l_ldim, m_ldim, r_ldim, nrow_i
!!$ Porgram starts here
!!$ -----------------------------------------------------------------------------
...
!!$ Brillouin zone information to be fixed
      ...
      if (opt%po%kind == 6) then
         ...
         If (opt%do_OAMdens /= 0) Then
           Call alloc_oam_density(oam, trgeo%tna, trgeo%ltna, trgeo%rtna)
           Call alloc_oam_density(oam1, trgeo%tna, trgeo%ltna, trgeo%rtna)
         Endif
      end if
...
!!$       Complex E fix
   ...
   Else !mpi_loc_id .ne. 0
      ...
      If (opt%do_OAMdens /= 0) Then
        Call alloc_oam_density(oam, 1, 1, 1)
        Call alloc_oam_density(oam1, 1, 1, 1)
      Endif
...
!      if (rhs%nrow .ne. 0 .and. rhs%ncol .ne. 0) then
!        ! rotate solution wave function basis
!        call alloc(wavefn_phase, rhs%nrow, rhs%nrow)
!        do nrow_i = 1,rhs%nrow
!          wavefn_phase%bl(nrow_i,nrow_i) = &
!&exp(DEF_cmplx_Ione*2*DEF_M_PI*RAND())
!        enddo
!
!        call alloc(temp_sol, rhs%nrow, rhs%ncol)
!        temp_sol%bl(:,:) = MATMUL(wavefn_phase%bl(:,:), rhs%bl(:,:))
!        rhs%bl(:,:) = temp_sol%bl(:,:)
!        call free(temp_sol)
!        call free(wavefn_phase)
!      endif
...
!!$ All postprocessing/output, which depends on sollution for local K-point comes here
      If (needsolve > 0) Then
         If (opt%po%kind == 6) then
            !if (opt%po%df > 0) then
            !   Call calc_torque_EMTO(rhs, trgeo_EMTO_nodf, rotm, tors1, l_ando%Nin*havelr, r_ando%Nin*haverl)
            !else
               Call calc_torque_EMTO(rhs, trgeo_EMTO, rotm, tors1, l_ando%Nin*havelr, r_ando%Nin*haverl)
            !end if
         Else
            Call calc_torque(rhs, trgeo, rotm, tors1, l_ando%Nin*havelr, r_ando%Nin*haverl)
            If (opt%do_OAMdens /= 0) Then
              Call calc_oam_density(rhs, trgeo, oam1, l_ando%Nin*havelr, r_ando%Nin*haverl)
            Endif
         End If

         Call bzsum_torq(tors, tors1, bz%kweight(ik))
         If (opt%do_OAMdens /= 0) Then
           Call bzsum_oam_density(oam, oam1, bz%kweight(ik))
         Endif
      ...
      !!$ interatomic currents 
      If (opt%do_iatcu /= 0 .or. opt%do_iatLcu /= 0) Then
         if (opt%po%kind == 6) then

            Call calc_iatcu_EMTO(rhs, msc, msdotc, atoms_EMTO%fermi, mpar_EMTO, trgeo_EMTO,&
              & rneibs, opt, iatcu1, k, l_ando%Nin*havelr, r_ando%Nin*haverl, opt%do_iatcu,&
              & opt%do_iatLcu, solve_comm)
         else
            Call calc_iatcu(rhs, msc, mpar, trgeo, rneibs, opt, iatcu1, k, &
             & l_ando%Nin*havelr, r_ando%Nin*haverl, opt%do_iatcu, opt%do_iatLcu, solve_comm)
         end if
         if (needsolve > 0) Call bzsum_iatcu(iatcu, iatcu1, bz%kweight(ik))
...
!!$             Call cond_spec (is1, is2, tran, lscell, rscell)
   ...
   If (opt%do_iatcu /= 0 .or. opt%do_iatLcu /= 0) Then
      Do itr = 1, iatcu%ntr
         Call MPI_Sum_conditional_d(iatcu%curten(itr)%a, iatcu1%curten(itr)%a, &
           &iatcu%curten(itr)%nnz*(2*iatcu%nch), hasdata, 0, solve_comm)
      End Do
      Call MPI_Sum_conditional_d(iatcu%cons, iatcu1%cons, iatcu%nat, hasdata, 0, solve_comm)
   End If

   If (opt%do_OAMdens /= 0) Then
    Call MPI_Sum_conditional_d(oam%lm, oam1%lm, 3*trgeo%tna, hasdata, 0, solve_comm)
    Call MPI_Sum_conditional_d(oam%rm, oam1%rm, 3*trgeo%tna, hasdata, 0, solve_comm)

    Call MPI_Sum_conditional_d(oam%Lt, oam1%Lt, 3*trgeo%rtna, hasdata, 0, solve_comm)
    Call MPI_Sum_conditional_d(oam%Lir, oam1%Lir, 3*trgeo%ltna, hasdata, 0, solve_comm)

    Call MPI_Sum_conditional_d(oam%Rt, oam1%Rt, 3*trgeo%ltna, hasdata, 0, solve_comm)
    Call MPI_Sum_conditional_d(oam%Rir, oam1%Rir, 3*trgeo%rtna, hasdata, 0, solve_comm)
   End If
   ...
   If (my_mpi_id == 0) Then
      ...
      End If
      If (opt%do_OAMdens /= 0) Then
        Call hdfwrite(h5out, 'oam', oam1)
        Call hdfwrite(h5out, 'oam/L-R', oam1%lm(:, :) - oam1%rm(:, :))
      Endif
   End If
