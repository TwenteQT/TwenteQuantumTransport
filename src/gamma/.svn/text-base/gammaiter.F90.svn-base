...
!!$ MPI vars
      Integer :: ierr, my_mpi_id, solve_comm, mpi_sz, boss, havejob, root = 0, solsize
...
!!$          flush (timef)
         If (opt%writeham == 1) Then
            Call hdfwrite (h5out, '/ham0', msys%c)
         End If
...
!!$ Solve system
      Call sp_solve_iter (msys%c, l_ando, r_ando, havelr+2*haverl, wf, root, solve_comm, opt%islv)
...
!!$ All postprocessing/output, which depends on solution for local K-poin comes here
      If (my_mpi_id == root) Then
         ...
         Call calc_torque (wf, trgeo, rotm, torqsigma, l_ando%Nin*havelr, r_ando%Nin*haverl)
         Call calc_zcurrent (wf, msys%c, trgeo, zcurr, l_ando%Nin*havelr, r_ando%Nin*haverl)

!!$ Calculate conductance
         cons = cond_full (tran, wf, l_ando, r_ando, havelr, haverl)
