...
Program qofz
   ...
   integer :: ik, nk
   type(rdensemat) :: rotm, magmom, rotx, roty
   ...
   logical :: kdep, log, pav, pav_input, cfav, doleads, plotall, ifoldunit, fixplane, flag, dis, gridscr, ofs
   integer :: scrtype !1 = perfect screening, 2 = naive Coulomb screening
   integer :: iar, i, err, icf, ncf
   double precision :: cfw
   double precision :: wsrav, max_zdif, cp_zdif, base(2, 2), scaling, area, perptr(3)
   type(planes_system) :: pl
   type(rdensemat) :: Loldc, Roldc, Loldc_av, Roldc_av, Loldc_cfav, Roldc_cfav
   type(t_interat_currents) :: iatcu

   type(confdata), Target :: cf, cf1, cf_pav, cfdummy1, cfdummy2
   type(confdisdata):: cf_dis
   type(confdisav):: cf_disav
   type(cell_system) :: cl
   type(confgrid):: cf_x, cf_y, cf_x1, cf_y1
   type(confgrid):: cf_xy, cf_x1y1
   Integer :: dpi
   ...
   Call reset_conf(cf)
   Call reset_conf(cf1)
   Call reset_conf(cfdummy1)
   Call reset_conf(cfdummy2)
   Call reset_conf(cf_pav)
   call reset_confdis(cf_dis)
   call reset_confdisav(cf_disav)
   call reset_confgrid(cf_x)
   call reset_confgrid(cf_y)
   call reset_confgrid(cf_xy)
   call reset_confgrid(cf_x1)
   call reset_confgrid(cf_y1)
   call reset_confgrid(cf_x1y1)
   log = .false.
   kdep = .false.
   pav = .true.
   pav_input = .false.
      ...
      do
         ...
         case ('-dpi')
            if (command_argument_count() > i) then
               call getarg(i + 1, cwork)
               read (cwork, '(I3)') dpi
               i = i + 1
               if (dpi > 100 .or. dpi < 1) then
                  write (*, '(x,4A)') "choose a dpi between 1 and 100!"
                  stop
               end if
            else
               dpi = 1
...
! do a loop over all the different geometries
   ...
   Do igeo = 1, ngeo
      ...
      Do icf = 1, nconf  ! loop over configurations to initialize and check consistency of options
         ...
         call hdfread(hid, '/opts/do_iatLcu', cf%do_iatlcu)
         call hdfread(hid, '/opts/do_OAMdens', cf%do_OAMdens)
         ...
         if (icf == 1) then
            ...
            if (pav) then
               ...
               cf1%do_OAMdens = cf%do_OAMdens
               cf1%do_iatlcu = cf%do_iatlcu
            ...
            cf1%do_OAMdens = min(cf1%do_OAMdens, cf%do_OAMdens)
            cf1%do_iatlcu = min(cf1%do_iatlcu, cf%do_iatlcu)
         end if
      ...
      Do icf = 1, nconf ! do the real work
         ...
         If (cf%do_OAMdens > 0) Then 
           Call hdfread(hid, '/oam', cf%OAMdens)
           Call scale_ts(cf%OAMdens)
         Endif
         ...
         if (scrtype == 1) call pscreen(cf%ts, cf%ts_pscr)
         if (scrtype == 2) call cscreen(cf%ts, cf%ts_pscr)
         If (cf%do_OAMdens > 0) Then 
           Call logscreen(log, 0, '... and orbital angular momentum densities ...')
           if (scrtype == 1) call pscreen_oam_density(cf%OAMdens, cf%OAMdens_pscr, cf%ts)
           if (scrtype == 2) stop 'NOT IMPLEMENTED!!'
         Endif
         ...
         if (scrtype == 1) call mudos(cf%ts, rotm, cf%spinmu, cf%spindos)
         if (scrtype == 2) call mudos_cscr(cf%ts, rotm, cf%spinmu, cf%spindos)
         ...
         if (pav) then
            ...
            cf_pav%do_OAMdens = cf%do_OAMdens
            cf_pav%do_iatlcu = cf%do_iatlcu
            ...
            If (cf_pav%do_OAMdens > 0) Then 
              call paverage(cf%OAMdens, cf_pav%OAMdens, pl)
              call paverage(cf%OAMdens_pscr, cf_pav%OAMdens_pscr, pl)
            Endif
            ...
            If (cf1%do_iatcu /= 0 .or. cf1%do_iatlcu /= 0) Then
               Call logscreen(log, 0, 'Processing interatomic currents ...')
               call hdfread(hid, '/current/iatcu', iatcu)
               call process_iatcu(iatcu, coords, pl, cf_pav, ifoldunit, dis, cl)
               if (scrtype == 1) call pscreen_cur(cf_pav)
               if (scrtype == 2) call cscreen_cur(cf_pav)
               !----------------
               if (dis) then
                  call hdfread(hid, '/geom/lgeo/sc_size', cl%sc, 2)
                  call hdfread(hid, '/geom/lgeo/l_perp_trans', perptr, 3)
                  call transform(cl, base, perptr)
                  call grid_dis(cl, dpi)
                  call getclwts(cl)
                  call process_gridcur(iatcu, ifoldunit, cf_pav%base, cf_pav%cond, cl, pl, cf_dis)
!             call write_dis(cf_dis,cl,bohr_in_nm,trim(dp%path)//trim('/zcurdis.dat'))
                  call makerotxy(rotx, roty, pl)

                  if (gridscr) then
                    if (scrtype == 1) then
                          call mudos(cf%ts, rotx, cfdummy1%spinmu, cfdummy1%spindos)
                          call mudos(cf%ts, roty, cfdummy2%spinmu, cfdummy2%spindos)
                    endif
                    if (scrtype == 2) then
                          call mudos_cscr(cf%ts, rotx, cfdummy1%spinmu, cfdummy1%spindos)
                          call mudos_cscr(cf%ts, roty, cfdummy2%spinmu, cfdummy2%spindos)
                    endif
                  else
                      call mudos_noscr(cf%ts, rotx, cfdummy1%spinmu, cfdummy1%spindos)
                      call mudos_noscr(cf%ts, roty, cfdummy2%spinmu, cfdummy2%spindos)
                  endif

                  Deallocate (rotx%bl, roty%bl)
                  call dis_quant(cf_dis%spinmuz, cf%spinmu, cl, pl)
                  call dis_quant(cf_dis%spinmux, cfdummy1%spinmu, cl, pl)
                  call dis_quant(cf_dis%spinmuy, cfdummy2%spinmu, cl, pl)

                  call makescr(cf_dis, cf_pav%ts%Lm(7, :), cf_pav%ts%Rm(7, :), cl)
                  call dis_quant(cf_dis%ts, cf%ts, cl, pl)
                  If (cf_pav%do_OAMdens > 0) then
                   call dis_quant(cf_dis%OAMdens, cf%OAMdens, cl, pl)
                  Endif

                  ! Testing the screening

                  call makegridcur(cf_dis, cl, cf_x, cf_y, cf_xy, bohr_in_nm)
                  if (gridscr) then
                     if (scrtype == 1) then
                        call pscr_grid(cf_x)
                        call pscr_grid(cf_y)
                        call pscr_grid(cf_xy)
                     endif
                     if (scrtype == 2) then
                        call cscr_grid(cf_x)
                        call cscr_grid(cf_y)
                        call cscr_grid(cf_xy)
                     endif
                  end if
                  call system('mkdir -p '//trim(dp%path)//'/grid/xgrid')
                  call system('mkdir -p '//trim(dp%path)//'/grid/ygrid')
                  call system('mkdir -p '//trim(dp%path)//'/grid/xygrid')
                  call alloc_gridata(cf_x1, cf_y1, cf_x1y1, cl)
                  if (ofs) then
                     call offset_cell(cf_x)
                     call offset_cell(cf_y)
                     call offset_cell(cf_xy)
                  end if
                  call writegridcur(cf_x, trim(dp%path)//trim('/grid/xgrid'))
                  call writegridcur(cf_y, trim(dp%path)//trim('/grid/ygrid'))
                  call writegridcur(cf_xy, trim(dp%path)//trim('/grid/xygrid'))
                  call add_gridata(cf_x1, cf_x, cfw)
                  call add_gridata(cf_y1, cf_y, cfw)
                  call add_gridata(cf_x1y1, cf_xy, cfw)
               end if
               !---------------
      ...
      End Do
      If (dis) Then
         call system('mkdir -p grid/xgrid')
         call system('mkdir -p grid/ygrid')
         call system('mkdir -p grid/xygrid')
         open (unit=2001, file='grid/grid.dat')
         write (2001, '(3I5)') cl%info
         Do i = 1, product(cl%info)
            write (2001, '(6g17.8e3)') cl%cell(1, i)*bohr_in_nm, cl%cell(2, i)*bohr_in_nm, cl%cell(3, i)*bohr_in_nm &
               , cl%dx(i)*bohr_in_nm, cl%dy(i)*bohr_in_nm, cl%dz(i)*bohr_in_nm
         End Do
         close (2001)
         open (unit=2001, file='grid/coord.dat')
         Do i = 1, size(cl%pts(1, :))
            write (2001, '(3g17.8e3)') cl%tr(1, i)*bohr_in_nm, cl%tr(2, i)*bohr_in_nm, cl%tr(3, i)*bohr_in_nm
         End Do
         close (2001)
         call writegridcur(cf_x1, 'grid/xgrid')
         call writegridcur(cf_y1, 'grid/ygrid')
         call writegridcur(cf_x1y1, 'grid/xygrid')
         call reset_confdis(cf_dis)
         call reset_confdisav(cf_disav)
         call reset_confgrid(cf_x)
         call reset_confgrid(cf_y)
         ...
         call reset_confgrid(cf_y1)
         call reset_confgrid(cf_x1y1)
         Deallocate (cl%cell, cl%pts, cl%tr, cl%zlr, cl%ind, cl%wt, cl%dx, cl%dy, cl%dz)
      End If
