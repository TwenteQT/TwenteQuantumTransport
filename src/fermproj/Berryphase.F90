...
!!$ 
   ...
   Real(Kind=DEF_DBL_PREC) :: kpar(2), kza
   Real(Kind=DEF_DBL_PREC) :: en, pi, rydberg
   Integer :: is, n, nh, j, nmod, i, ordered(100)
   Integer :: bandf = 33
   ...
   Real(Kind=DEF_DBL_PREC) :: k1, dffact
...
#ifdef _VERS_
   Call do_log(1, 'Berry phase code v'//_VERS_//' by Msrang')
#else
   Call do_log(1, 'Berry phase code vXXXX by Msrang')
#endif
...
!!$       Allocate (jobs(mpi_sz))
   If (my_mpi_id == root) Then
      Log_Level = opt%loglvl
      Open (Unit=bandf, File='plotFermi.b', Action='write')
   Else
...
!!$  Solve ando problem
       Call solve_ando(ofs, hops, n, nh, ando, aopts)
          ...
          nmod = ando%Nin + ando%Nout
          bz%bz(ik)%nmod = nmod 

          If (.not. Allocated(bz%bz(ik)%kz) .and. nmod /= 0) then
           Allocate(bz%bz(ik)%kz(nmod)) !Now that we know how many kz solutions are associated with this kx,ky
                                       ! we can allocate this
           Allocate(bz%bz(ik)%vec(n,nmod)) !eigenvectors for this kx,ky
           Allocate(bz%bz(ik)%connect(9,nmod)) ! connectivity
           Allocate(bz%bz(ik)%s_z(nmod)) !spin character of eigvec (only set after diagonalize_degenerate_subspace is called)
           bz%bz(ik)%kz(:) = 0
           bz%bz(ik)%connect = 0
           bz%bz(ik)%s_z = 0
          Endif
          ...
          if (nmod==0) Then 
            Allocate(bz%bz(ik)%kz(1))
            bz%bz(ik)%kz(:) = 0
          else
          
            do i=1,ando%Nin
              bz%bz(ik)%kz(i) =(imag(Log(ando%lin(i))))/kza
              bz%bz(ik)%vec(1:n,i) = ando%Uin(1:n,i)
            enddo

         
            do i=1,ando%Nout
              bz%bz(ik)%kz(i+ando%Nin) =(imag(Log(ando%lout(i))))/kza
              bz%bz(ik)%vec(1:n,i+ando%Nin) = ando%Uout(1:n,i)
            enddo
          Endif

          k(1) = en
          k(2) = real(nmod, kind=DEF_DBL_PREC)
          Do i = 1, nmod, 1
             k(i + 2) = bz%bz(ik)%kz(i)
          End Do

          If (opt%po%kind .Le. 3) Then
             k(nmod + 3:bsc%nrows + 2) = 0.0 !!$/ 0.0
          Else If (opt%po%kind > 4) Then
             k(nmod + 3:bsc_EMTO(1)%ldim + 2 - size(bgeo_EMTO%to_downfold)/2) = 0.0 !!$/ 0.0
          End If
   ...
   Call find_bz_degenerate(bz)
   Call diagonalize_degenerate_subspace(bz)
   Call compute_bz_connect(bz, opt%bzo%tol)
!   Call print_bz_as_mesh(bz)

   !Actually calculate Berry phase things
   SHC = compute_HC(bz, SPIN)
   print*,'spin Hall conductivity is ',SHC
...
Contains
   Subroutine log_bands(comm, root, kpar, k, n, bandf)
      Implicit None
      Integer :: root, bandf, n, comm
      Real(Kind=DEF_DBL_PREC) :: kpar(2), k(n)
!!$ Locals
      ...
      Else
            nmod = Int(k(2))
            Do j = 1, nmod
                Write (bandf, '(3(1x,f12.8))') kpar(1), kpar(2),  k(j + 2)
            End Do
         Do procnum = 2, mpi_sz
            Call MPI_Recv(k, n, MPI_DOUBLE_PRECISION, procnum - 1, procnum + mpi_sz + 999, comm, status, &
           & ierr)
               nmod = Int(k(2))
               Do j = 1, nmod
                 Write (bandf, '(3(1x,f12.8))') kpar(1), kpar(2),  k(j + 2)
