!!$ $Id: potpars.F90 1990 2013-10-31 09:32:04Z rien $
#include "math_def.h"
Module potpars
      Use sparselib
      Implicit None
      ...
      Type t_pot_opts
         ...
         Integer :: nonsph
         Integer :: df
         Real (Kind=DEF_DBL_PREC) :: L_shift, R_shift
         !Integer :: l_lfilt, r_lfilt
...
!!$ local vars
      ...
      Function calc_magmoms_EMTO (geo, rotm, magvals, magmom) Result (f1)
         Use geometry_module
         Implicit None
...
!!$ local vars
         ...
         If ( .Not. f1) Then
            ...
            magmom = 0.0d0
         End If
      End Function calc_magmoms_EMTO
...
!SMP$ DO SERIAL
      ...
      Function prep_rot_mask_EMTO (geo, po) Result (rotm)
         Use logging
         Use rotations
         Use geometry_module
         Implicit None
...
!SMP$ DO SERIAL
         ...
         Else
            ...
            End Do
         End If
      End Function prep_rot_mask_EMTO
...
!!$ Local
         ...
         Call alloc (ppar%bdotl, geom%norbit*2, geom%norbit*2)

         ii1 = 1
         Do n = 1, geom%num
            aptr => geom%atoms(n)%ptr
!!$ Set overlap, sqrtd and pfun matrices
            Do s = 1, 2
               Do l = 1, aptr%nl
                  ii2 = ii1 + 2 * (l-1)
                  ...
                  ii1 = ii2 + 1
               End Do
               Call insert_densemat_into_zcsr(aptr%pp(s)%el%bdotl(:,:), ppar%bdotl)
            End Do
         End Do
         ...
         If (present(rotm)) Then
            ...
            Call RotateZCSR (oph, ppar%bdotl, op, po%rnegl)
...
!!$ Local vars
         ...
         Complex (Kind=DEF_DBL_PREC), Allocatable, Dimension (:) :: A
         Integer, Allocatable, Dimension (:) :: jc, ir
         nnz = 0

         Do i = 1, amat%nnz
            If (Abs(amat%a(i)) > zcrit) Then
               nnz = nnz + 1
            Else
               amat%jc (i) = - 99
            End If
         ...
         If (nnz < amat%nnz) Then
            ...
            Do j = 1, amat%nrow
               l = ir (j)
               Do i = amat%ir (j), amat%ir (j+1) - 1
                  If (amat%jc(i) /=-99) Then
                     jc (l) = amat%jc (i)
                     A (l) = amat%a (i)
                     l = l + 1
                  End If
            ...
            Allocate(amat%a(amat%nnzmax),amat%ir(size(ir)),amat%jc(amat%nnzmax))
            amat%A(1:size(A)) = A(:)
            amat%jc(1:size(jc)) = jc(:)
            amat%ir(1:size(ir)) = ir(:)
            amat%nnz = nnz
            Deallocate (ir, jc, A)
         End If
...
!!$ Local
      ...
      Subroutine GetRotOp_EMTO (op, geom, mask)
         Use geometry_module
         Use rotations
         Implicit None
