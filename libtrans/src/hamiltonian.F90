!!$ $Id: hamiltonian.F90 1878 2013-04-08 12:54:47Z antst $
#include "math_def.h"
Module hamiltonian
      Use sparselib
      Use potpars
     ...
     & prep_system_sc_EMTO, prep_system_sk_EMTO
      End Interface
...
!!$          Type (zcsrmat) :: m1, m2
      ...
      Subroutine prep_system_sk_EMTO (ham, sk, ppar, po, needhops)
         Use structure         
         Implicit None
         Type (t_omta_logder) :: ppar 
         Integer :: needhops
         Type (t_mathop) :: ham, sk
         Type (t_pot_opts) :: po
         Select Case (po%kind)
         Case (handle_EMTO_system)
            Call make_h_sk_EMTO (ham, sk, ppar, needhops)
         Case Default
            Write (*,*) 'I"m not smart enough to handle system'
            Write (*,*) 'with identifier:', po%kind, 'Fix ME! :)'
            Stop
         End Select
         Return
      End Subroutine prep_system_sk_EMTO

      Subroutine prep_system_sc_EMTO (ham, sc, k, ppar, po, needhops)
         Use structure
         Implicit None
         Type (t_omta_logder) :: ppar
         Integer :: needhops
         ...
         Select Case (po%kind)
         Case (handle_EMTO_system)
            Call make_h_sc_EMTO (ham, sc, k, ppar, needhops)
         Case Default
            Write (*,*) 'I"m not smart enough to handle system'
            Write (*,*) 'with identifier:', po%kind, 'Fix ME! :)'
            Stop
         End Select
         Return
      End Subroutine prep_system_sc_EMTO
...
!!$             m1 = make1cent (ham%c, ppar%c1conj, ppar%c1)
         ...
         m1 = spmatadd (ham%c, ppar%bdotl)
         call spcopy(m1, ham%c)
         Call free (m1)
...
!!$ Local
         Type (zcsrmat) :: H
         Type (zcsrmat) :: m1, m2, mtx2

         H = getH (sk%c, ppar, 1)
         m1 = make1cent (H, ppar%c1, ppar%c1conj)
         Call free (ham%c)
         m2 = spmatadd (m1, ppar%bdotl)
         ham%c = spmatadd (m2, ppar%c0)
         Call free (m1)
         Call free (m2)
      ...
      Subroutine make_h_sc_EMTO (ham, sc, k, ppar, needhops)
         Use structure
         Implicit None
         Type (t_omta_logder) :: ppar
         Type (t_strconst) :: sc
...
!!$ Local
         ...
         Call make_h_sk_EMTO (ham, sk, ppar, needhops)
         Call free_mathop (sk)
      End Subroutine make_h_sc_EMTO

      Subroutine make_h_sk_EMTO (ham, sk, ppar, needhops)
         Use structure
         Implicit None
         Type (t_omta_logder) :: ppar
         Integer :: needhops
         Type (t_mathop) :: ham, sk
!!$ Local
         Type (zcsrmat) :: intermediate

         intermediate = spmatadd(ppar%hsoc, ppar%logder, sign=1)
         
         ham%c = spmatadd(sk%c, intermediate, sign=1)
         ...
         call free(intermediate)

      End Subroutine make_h_sk_EMTO
