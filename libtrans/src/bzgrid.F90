!!$ $Id: bzgrid.F90 1974 2013-09-09 18:19:13Z antst $
#include "math_def.h"
Module bzgrid
...
!!$ THE KX=0 OR KY=0
         ...
         Else
            istart = 1
         End If
         stx = real ((2*nk1-1), kind=DEF_DBL_PREC)
         sty = real ((2*nk2-1), kind=DEF_DBL_PREC)
...
!	  end do
         Do i1 = istart, nk1-1
!!$      DO 300 I1=1,NK1
            fx1 = (real(i1, kind=DEF_DBL_PREC)) / stx
!!$       fx1 = (real(i1,kind=DEF_DBL_PREC)) / sty
            Do i2 = - nk2 + 1, nk2-1
!!$      DO 300 I2=1,NK2
               fx2 = (real(i2, kind=DEF_DBL_PREC)) / sty
!!$          fx2 = (real(i2,kind=DEF_DBL_PREC)) / sty
!                if (dsqrt((fx1 * vbg (1, 1) + fx2 * vbg (1, 2))**2d0+(fx1 * vbg(2, 1) + fx2 * vbg (2, 2))**2d0) <= dsqrt(vbg (1, 1)**2d0+vbg(2,1)**2d0)/2) then
               ibz = ibz + 1
