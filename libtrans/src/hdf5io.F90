...
Module hdf5io
      ...
      Interface hdfwrite
        ...
        & hdfwrite_geo, hdfwrite_geo_EMTO, hdfwrite_bz, hdfwritezmat, hdfwritezvec, hdfwritezcsrmat, hdfwritercsrmat, &
        ...
        & hdfwriteDAMPopts, hdfwriteAndo, hdfWriteOAMDensity
!, hdfwritespatialquantdis ,hdfwritecoordis
      End Interface
      ...
      Interface hdfread
        ...
        & hdfReadZMat3, hdfread_iatcu!, hdfReadSuperCellSize
...
!!$ Local
         ...
         Call hdfwrite (gid, 'do_iatLcu', opt%do_iatLcu)
         Call hdfwrite (gid, 'do_OAMdens', opt%do_OAMdens)
...
!!$   Local vars
   ...
   End Subroutine hdfwrite_geo
      ...
      Subroutine hdfwrite_geo_EMTO (tgt, path, geo)
         Use geometry_module
         Implicit None
...
!!$   Local vars
         ...
         Call hdfwrite (tgt, path//'/magmom', coord(:, 1))
         Deallocate (coord)
   End Subroutine hdfwrite_geo_EMTO
...
!!$ 'm' is middle region, 'ir' is incident plus reflected, 't' is transmitted.
   ...
   Subroutine hdfWriteOAMDensity (tgt, path, ts)
         Use OAMdensity
         Implicit None
         Integer (HID_T), Intent (In) :: tgt
         Character (Len=*), Intent (In) :: path
         Type (OAM_density), Intent (In) :: ts
!!$           Local vard
         Integer (HID_T) :: grp_id
         Integer :: error
         Call hdfcheckgrp (tgt, trim(adjustl(path)))
...
!!$ 'm' is middle region, 'ir' is incident plus reflected, 't' is transmitted.
         ...
         Call h5gclose_f (grp_id, error)
   End Subroutine hdfWriteOAMDensity
...
!!$ Local vars
   ...
   End Subroutine hdfset_write_torque_parallel
      ...
      Subroutine hdfwrite_iatcu (tgt, path, iatcu)
         ...
         Call hdfwrite (grp_id0, 'nch', iatcu%nch)
...
!!$       End Subroutine writetextattr
  ...
  & xfer_prp)
         ...
         Interface
           ...
           & xfer_prp_default, buf, dims) BIND(C,name="h5dwrite_f_c")
               Use H5GLOBAL
...
!!$          Complex (Kind=DEF_DBL_PREC), Intent (Inout), Dimension (*) :: buf(:,:) ! data buffer
         ...
         Interface
            Integer Function h5ltread_dataset_dl2_c (loc_id, namelen, dset_name, type_id, buf, dims) &
                    & BIND(C,name="H5LTread_dataset_double")
               Use H5GLOBAL
...
!!$           Local vard
   ...
   !Subroutine hdfReadSuperCellSize (tgt, path, sc_size)
   !      Use sparselib
   !      Use geometry_module
   !      Implicit None
   !      Integer (HID_T), Intent (In) :: tgt
   !      Character (Len=*), Intent (In) :: path
   !      Integer :: sc_size(2)
   !      Integer (HSIZE_T) :: dims (2), type_size
   !      Integer :: error, type_class
   !     
   !      Call h5ltget_dataset_info_f (tgt, path, dims, type_class, type_size, error)
   !      Call H5LTread_dataset_int_f (tgt, path, sc_size, dims, error)

   !End subroutine hdfReadSuperCellSize
   ...
   End Subroutine hdfReadSpaqu
      ...
      Subroutine hdfread_iatcu (tgt, path, iatcu)
         ...
         Integer :: error, buf (1), nat, ntr, nch, nnz, itr
         Integer (HSIZE_T) :: dims (2)
         Character (Len=10) :: cwork
         ...
         ntr = buf (1)
         Call h5ltread_dataset_f (grp_id0, 'nch', H5T_NATIVE_INTEGER, buf, dims, error)
         if (error .ge. 0) then
           nch = buf (1)
         else
           nch = 4
         ...
         Do itr = 1, ntr
            ...
            nnz = buf (1)
            Call alloc_curten(ctp, nnz, nat, nch)
            dims(2) = nat+1
