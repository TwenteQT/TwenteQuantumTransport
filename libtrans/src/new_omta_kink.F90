...
!! Local variables:
   ...
   nbas=geo%num
   call alloc(sf,scalpha%ns+scalpha%ldim,scalpha%ldim)
   !!$call alloc(s0,scalpha%ns+scalpha%ldim,scalpha%ldim)
   if(need_hop==1)then
     call alloc(hf,scalpha%ns+scalpha%ldim,scalpha%ldim)
     !!$call alloc(j0,scalpha%ns+scalpha%ldim,scalpha%ldim)
     hop=.true.
   endif
      ...
      if (present(tr)) then
       ...
       do jlm = 1, nlsqrj
         ...
         do ilm = 1,nlsqri
            ...
            if (abs(scalpha%s(offs)*cost) < nzero .and. abs(scalpha%s(offs)*sint) < nzero) cycle
...
!!$ now to add logder
   ...
   sf_csr = conv2csr(sf)
   s0 = spmatadd(logder,sf_csr)
 !$  s0 = conv2csr(sf)
   if(hop) j0 = conv2csr(hf)

 call dump_matrix(s0,'s0')
