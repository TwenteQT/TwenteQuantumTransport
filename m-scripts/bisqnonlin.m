function [p,dp,opi]=bisqnonlin(mf1,x1,y1,pin,opts)
    ...
    [f,p0,exitflag,lsiter,corp,covp,covr,stdresid,Z,r2]=...
    leasqr(x1,y1,pin,mf1,Tol,MaxIter,sqrtwt,dp,'dfdp',opt);
    p = p0(:);
    totaliter = lsiter;
    res=y1-f;
%    size(x1)
%    size(y1)
%    size(f)
%    plot(x1,reshape(y1,length(x1),3),'r.',x1,reshape(f,length(x1),3),'.k');
    NP=length(p);
    resin=res;
    ...
    jacob=dfdp(x1,f,p,dp,mf1,opt.bounds);
    [Q,ignore]=qr(jacob,0);
...
%    sigma = norm(resin) / sqrt(dfeu);
    ...
    origMaxIter=MaxIter;
    iterlim=origMaxIter-lsiter;

    res=resin;
    if (isRobust==1)&& (iterlim>0)
        while true
            robiter = robiter + 1;
